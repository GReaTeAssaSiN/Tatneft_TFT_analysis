...
│   ├── 5stations_metadata.csv   — паспорт 5 АЗС (тип дороги, услуги, кол-во колонок)
...
│
├── tft/
│   ├── model.ckpt               — лучшая обученная модель (обновляется при новом best val_loss)
│   ├── train_config.json        — гиперпараметры (создаётся train_dashboard, необязателен)
│   ├── prepare_dataset.py      → training_dataset.pkl + dataset_config.pkl
│   ├── train.py                → model.ckpt
│   ├── predict.py              → data/predictions.csv + data/metrics.csv
│   ├── checkpoints/             — чекпоинты Lightning (хранится лучший по val_loss)
│   └── logs/                    — TensorBoard логи (tensorboard --logdir tft/logs)
│
├── dashboard/
│   ├── app_dashboard.py         — итоговый дашборд: EDA + прогнозы + интерпретация (4 вкладки)
│   └── train_dashboard.py       — дашборд обучения: гиперпараметры, запуск, результаты
│
├── utils/
